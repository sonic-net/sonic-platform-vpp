...
format_ip6_validate_trace (u8 *s, va_list *args)
{
  ...
  ip6_validate_trace_t *t =
    va_arg (*args, ip6_validate_trace_t *);

  s = format (s,
	      "IP6-VALIDATE: sw_if_index %d next %d src %U dst %U",
...
ip6_validate_trace_one (vlib_main_t *vm, vlib_node_runtime_t *node,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  for (int i = 0; i < IP6_VALIDATE_N_ERROR; i++)
    {
      if (error_counts[i])
	vlib_node_increment_counter (vm, ip6_validate_node.index, i,
				     error_counts[i]);
    }
